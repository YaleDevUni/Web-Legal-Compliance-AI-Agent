...
def _make_citation_from_meta(meta: dict) -> Citation:
    ...
    try:
        updated_at = datetime.fromisoformat(
            updated_at_raw) if updated_at_raw else datetime(2024, 1, 1)
    except ValueError:
...
def _parse_llm_response(
    ...
    for line in content.strip().split("\n"):
        ...
        source_location = _find_source_location(
            code_text, code_snippet) if code_snippet else None
...
class BaseAgent:
    ...
    def analyze(self, code_text: str, search_query: str | None = None) -> list[ComplianceReport]:
        ...
        if self._retriever is not None:
            ...
            logger.debug(
                f"[{agent_name}] 검색된 Top-K 문서 (필터링 전): {raw_chunks_log}")
            ...
            if self._RELEVANT_PREFIXES:
                ...
                logger.debug(
                    f"[{agent_name}] 필터링 후 최종 문서: {filtered_chunks_log}")
            else:
                chunks = raw_chunks
        ...
        # 5. 파싱
        reports = _parse_llm_response(
            response.content, chunk_index, code_text)  # Changed variable name
        # Changed log message
        logger.info(f"[{agent_name}] 분석 완료: {len(reports)}개의 보고서 생성됨")
